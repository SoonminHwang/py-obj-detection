...
if __name__ == '__main__':
	...
	cfg_from_file('experiments/cfgs/faster_rcnn_end2end_kaist.yml')
	...
	height, width = (int(512*im_scale/16), int(640*im_scale/16))
	feat_stride = 16
	...
	inds_inside = np.where(
        (all_anchors[:, 0] >= 0) &
        (all_anchors[:, 1] >= 0) &
        (all_anchors[:, 2] < 640) &  # width
        (all_anchors[:, 3] < 512)    # height
	)[0]
	...
	anchor_hist = np.zeros( (A), dtype=np.uint)	
	for i in xrange(len(imdb.image_index)):
		...
		print 'bbox_transform_inv: ', bbox_transform_inv(box, delta)			
		print 'gt_boxes: ', gt_boxes.astype(np.uint16)


		### Assigned anchor histogram
		# anchor_hist[ np.asarray(idx % A) ] += 1
		for ii in idx:
			anchor_hist[ii%A] += 1

	print 'Assigned anchor histogram: '
	print anchor_hist
	...
	# 	import cv2
	# 	img = cv2.imread( imdb.image_path_at(i)[0] )[:,:,(2,1,0)]
