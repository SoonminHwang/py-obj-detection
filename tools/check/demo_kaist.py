...
if __name__ == '__main__':
    ...
    resDir = os.path.join(cfg.ROOT_DIR, 'output', 'kaist_2015_test20', 
        nowStr, caffemodel.split('/')[-1].split('.')[0])
    if not os.path.exists(resDir):
        os.makedirs( resDir )

    from datasets.kaist import kaist
    imdb = kaist('test20', '2015')
    roidb = imdb.roidb

    cfg_from_list(['TEST.SCALES', '[1024]'])
    # for ii in range(5):
    import numpy.random as npr    
    for ii in npr.choice(imdb.num_images, size=(5), replace=False):
