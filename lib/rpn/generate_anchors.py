...
def kitti_kmeans_anchors_ped_cyc_2x(num_anchors):
    if num_anchors == 10:
        anchors = np.array([[  -9.03,  -27.28,    9.03,   27.28],
                           [ -14.81,  -40.48,   14.81,   40.48],
                           [ -20.34,  -58.41,   20.34,   58.41],
                           [ -35.77,  -42.85,   35.77,   42.85],
                           [ -30.29,  -77.23,   30.29,   77.23],
                           [ -42.14, -106.9 ,   42.14,  106.9 ],
                           [ -75.92,  -88.81,   75.92,   88.81],
                           [ -60.67, -150.84,   60.67,  150.84],
                           [ -89.09, -185.57,   89.09,  185.57],
                           [-170.29, -196.99,  170.29,  196.99]])
    else:
...
def kitti_kmeans_anchors_4x(num_anchors):
    # imdb
    # 'hRng' : [20, np.inf], # Min. 20 x 50 or 25 x 40
    # 'occLevel' : [0, 1, 2],       # 0: fully visible, 1: partly occ, 2: largely occ, 3: unknown
    # 'truncRng' : [0, 0.5]
    if num_anchors == 20:
        anchors = np.array([[ -18.47,  -55.06,   18.47,   55.06],
		       [ -51.53,  -46.85,   51.53,   46.85],
		       [ -29.89,  -86.19,   29.89,   86.19],
		       [ -72.74,  -49.72,   72.74,   49.72],
		       [ -43.8 , -119.77,   43.8 ,  119.77],
		       [ -82.99,  -68.95,   82.99,   68.95],
		       [-120.86,  -52.66,  120.86,   52.66],
		       [-111.24,  -87.83,  111.24,   87.83],
		       [ -65.9 , -163.59,   65.9 ,  163.59],
		       [-170.33,  -70.4 ,  170.33,   70.4 ],
		       [-149.02, -113.19,  149.02,  113.19],
		       [-234.42,  -94.25,  234.42,   94.25],
		       [-100.65, -241.64,  100.65,  241.64],
		       [-206.04, -149.15,  206.04,  149.15],
		       [-318.81, -130.2 ,  318.81,  130.2 ],
		       [-163.24, -350.07,  163.24,  350.07],
		       [-298.28, -202.98,  298.28,  202.98],
		       [-468.43, -187.93,  468.43,  187.93],
		       [-438.81, -298.22,  438.81,  298.22],
		       [-675.75, -369.29,  675.75,  369.29]])
...
def kitti_kmeans_anchors_8x(num_anchors):
    # imdb
    # 'hRng' : [20, np.inf], # Min. 20 x 50 or 25 x 40
    # 'occLevel' : [0, 1, 2],       # 0: fully visible, 1: partly occ, 2: largely occ, 3: unknown
    # 'truncRng' : [0, 0.5]
    if num_anchors == 20:
        anchors = np.array([[  -39.71,  -118.15,    39.71,   118.15],
		       [ -102.51,   -93.22,   102.51,    93.22],
		       [  -69.08,  -195.25,    69.08,   195.25],
		       [ -143.71,  -100.33,   143.71,   100.33],
		       [ -166.27,  -138.38,   166.27,   138.38],
		       [ -233.98,  -103.16,   233.98,   103.16],
		       [ -109.72,  -282.93,   109.72,   282.93],
		       [ -221.3 ,  -174.98,   221.3 ,   174.98],
		       [ -328.98,  -136.43,   328.98,   136.43],
		       [ -294.06,  -223.69,   294.06,   223.69],
		       [ -165.19,  -402.65,   165.19,   402.65],
		       [ -455.73,  -183.58,   455.73,   183.58],
		       [ -403.33,  -291.84,   403.33,   291.84],
		       [ -247.97,  -583.13,   247.97,   583.13],
		       [ -624.9 ,  -254.45,   624.9 ,   254.45],
		       [ -576.15,  -393.4 ,   576.15,   393.4 ],
		       [ -375.46,  -752.68,   375.46,   752.68],
		       [ -922.38,  -366.98,   922.38,   366.98],
		       [ -851.71,  -561.24,   851.71,   561.24],
		       [-1296.82,  -735.18,  1296.82,   735.18]])
